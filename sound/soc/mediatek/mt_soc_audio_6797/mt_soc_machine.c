...
static struct snd_soc_dai_link mt_soc_dai_common[] = {
...
#ifdef Sharp_Z2
	{
		...
		.cpu_dai_name   = MT_SOC_I2S0DL1_NAME,
		.platform_name  = MT_SOC_I2S0DL1_PCM,
		.codec_dai_name = MT_SOC_CODEC_I2S0DL1_DUMMY_NAME,
		.codec_name = MT_SOC_CODEC_DUMMY_NAME,
	},
#endif
};
...
static struct snd_soc_dai_link mt_soc_extspk_dai[] = {
	{
		...
		.stream_name = MT_SOC_SMARTPA_PLAYBACK_STREAM_NAME,
		.cpu_dai_name   = "snd-soc-dummy-dai",
	...
	{
		.name = "ext_Smartpa_Capture",
		.stream_name = MT_SOC_SMARTPA_CAPTURE_STREAM_NAME,
		.cpu_dai_name   = MT_SOC_I2S0AWBDAI_NAME,
		.platform_name  = MT_SOC_I2S0_AWB_PCM,
		.codec_dai_name = "tfa98xx-aif",
		.codec_name = "tfa98xx.2-0034",
...
static int __init mt_soc_snd_init(void)
{
	...
	if (node != NULL) {
		memcpy(mt_soc_dai_component + daiLinkNum, mt_soc_extspk_dai, sizeof(mt_soc_extspk_dai));
		daiLinkNum +=ARRAY_SIZE(mt_soc_extspk_dai);
	} else {
		pr_err("tfa98xx is not find");
	}
#endif
#else
...
//only for test
	...
	mt_sco_audio_debugfs = debugfs_create_file(DEBUG_ANA_FS_NAME,
	   0666, NULL, (void *) DEBUG_ANA_FS_NAME, &mtaudio_ana_debug_ops);
#else
	/* create debug file */
	mt_sco_audio_debugfs = debugfs_create_file(DEBUG_FS_NAME,
